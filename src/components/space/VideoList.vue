            ...
            <div class="v-filter-line clearfix">
                <div class="be-tab">
                    <ul class="be-tab-inner clearfix">
                        <li class="be-tab-item" @click="filterVideo('new')" :class="filter === 'new' ? 'is-active' : ''">
                            <a name="be-tab-1" class="be-tab-input"></a>
                            <span>最新发布</span>
                        </li>
                        <li class="be-tab-item" @click="filterVideo('mostPlay')" :class="filter === 'mostPlay' ? 'is-active' : ''">
                            <a name="be-tab-1" class="be-tab-input"></a>
                            <span>最多播放</span>
                        </li>
                        <li class="be-tab-item" @click="filterVideo('mostCollect')" :class="filter === 'mostCollect' ? 'is-active' : ''">
                            <a name="be-tab-1" class="be-tab-input"></a>
                            <span>最多收藏</span>
                        </li>
                    </ul>
...
export default {
    ...
    methods: {
        getVideo() {
            spaceApi.spaceVideo({ 'userId': this.userInfo.iD, 'pageNum': this.pageNum, 'pageSize': this.pageSize }).then((response) => {
                this.videoList = response.list
                this.total = response.total
            })
        },
        getMostPlay() {
            spaceApi.mostPlay({ 'userId': this.userInfo.iD, 'pageNum': this.pageNum, 'pageSize': this.pageSize}).then((response) => {
                this.videoList = response.list
                this.total = response.total
            })
        },
        getMostCollect() {
            spaceApi.mostCollect({ 'userId': this.userInfo.iD, 'pageNum': this.pageNum, 'pageSize': this.pageSize}).then((response) => {
                this.videoList = response.list
                this.total = response.total
            })
        },
