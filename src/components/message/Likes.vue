            ...
            <ul>
                <li v-for="item in list" :key="item.id">
                    <a :href="(item.comment_id === '' || item.comment_id === undefined) ? '/video/'+item.video_id+'#playerWrap' : '/video/'+item.video_id+'#'+item.comment_id" target="_blank" class="item" @click="watch">
                        ...
                        <div class="con">
                            <div class="user">
                                <a :href="'/ospace/index?id='+item.author_id" target="_blank" class="name vip-red-name">{{item.nick_name}}</a>
                                <span v-show="item.comment_id !== '' && item.comment_id !== undefined">
                                    点赞了我的评论：
                                    <div class="parent-content">{{item.comment_content}}</div>
                                </span>
                                <span v-show="item.comment_id === '' || item.comment_id === undefined">
                                    点赞了我的视频：
                                    <div class="parent-content">{{item.video_title}}</div>
                                </span>
                            </div>
                            <div class="info">
...
export default {
    ...
    methods: {
        getData() {
            messageApi.getLike({ 'beLikedUserId': this.userInfo.iD, 'pageNum': this.pageNum, 'pageSize': this.pageSize }).then((response) => {
